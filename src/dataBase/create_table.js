...
const sqlQueriesEnum = [
	{
		...
		createUserBlocage: `CREATE TABLE Blocages (
		...
		createUserFollow: `CREATE TABLE Suivis (
		...
		createWebsiteStats: `CREATE TABLE Statistiques (
...
const initiDatabase = Object.values(sqlQueriesEnum).join(' ')
...
db.serialize(() => {
	db.exec(
        ...
        site_web VARCHAR,
        avatar_id INTEGER,
        role TEXT CHECK(role IN ('artiste', 'utilisateur', 'admin')),
        date_inscription DATETIME,
        conditions_generales BOOLEAN,
        newsletter BOOLEAN,
        FOREIGN KEY (avatar_id) REFERENCES Avatars(avatar_id)
